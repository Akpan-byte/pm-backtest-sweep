    ...
    with gzip.open(path, "rt", encoding="utf-8") as fh:
        for raw in fh:
            ...
            if not line:
                continue
            def _keep(obj):
                return {"pnl": obj.get("pnl"), "opened_at": obj.get("opened_at"), "closed_at": obj.get("closed_at")}
            try:
                out.append(_keep(json.loads(line)))
            except json.JSONDecodeError:
                recovered = []
                cursor = 0
                while cursor < len(line):
                    for end in range(len(line), cursor, -1):
                        try:
                            obj = json.loads(line[cursor:end])
                            recovered.append(_keep(obj))
                            cursor = end
                            break
                        except json.JSONDecodeError:
                            continue
                ...
                if recovered:
                    skipped += 1
    if skipped:
        print(f"WARN {path}: skipped/reconstructed {skipped} malformed line(s)", file=sys.stderr)
    return out
