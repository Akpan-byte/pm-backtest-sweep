...
def breakout_pct_008_signal(
    ...
    if rem_sec <= 5 or rem_sec >= duration - 5:
        return {
            ...
            "entry_price": 0.0,
            "source": "BREAKOUT_PCT_0.08",
            "reason": f"Filtered: rem_sec {rem_sec} outside range (5, {duration - 5})",
        }

    if spot_price >= upper or z_score >= 0.5:
        direction = "YES"
        reason = f"Spot {spot_price:.2f} >= Upper {upper:.2f} (0.08% breakout) or z_score={z_score:.2f}"
    elif spot_price <= lower or z_score <= -0.5:
        direction = "NO"
        reason = f"Spot {spot_price:.2f} <= Lower {lower:.2f} (0.08% breakout) or z_score={z_score:.2f}"
