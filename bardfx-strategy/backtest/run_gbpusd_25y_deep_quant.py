#!/usr/bin/env python3
"""
GBP/USD 25-Year Master Quantitative Backtesting & Risk Study (2001 - 2026)
========================================================================
...
- 5-Fold Walk-Forward Chronological Stability Splits (5-Year intervals)
...
def load_data():
    ...
    df_merged['dt_date'] = df['timestamp_ny'].dt.date
    df_merged['dt_hour'] = df['timestamp_ny'].dt.hour
    df_merged['dt_minute'] = df['timestamp_ny'].dt.minute
    df_merged['dt_weekday'] = df['timestamp_ny'].dt.weekday
...
def run_simulation(df):
    ...
    print(f"Executing 25-year chronological backtest loop starting from index {start_idx}...")
...
def main():
    print("=" * 80)
    print("GBP/USD 25-YEAR MASTER HISTORICAL DEEP QUANTITATIVE STUDY")
    print("=" * 80)
    ...
    print(f"Simulation completed. Total trades processed: {n_trades:,}")
    
    if n_trades == 0:
        print("Error: No trades executed in the backtest.")
        return
    ...
    print("\nRunning 10,000-run vectorized Monte Carlo simulations...")
    mc_results = run_monte_carlo(trades_comp)
    
    print("Running chronological 5-Fold Walk-Forward stability splits...")
    wfo_results = run_walk_forward_stability(trades_comp)
    
    # Output to stdout
    print("\n" + "="*80)
    print("MASTER 25-YEAR DEEP QUANT METRICS: GBP/USD (2001 - 2026)")
    print("="*80)
    print(f"  Active Trading Days:  {n_days:,} days (~25.0 years)")
    print(f"  Total Trades:         {n_trades:,}")
    print(f"  Avg Trades per Day:   {n_trades / n_days:.4f} trades/day")
    print(f"  Win Rate:             {win_rate:.2f}% ({len(wins):,} Wins, {n_trades - len(wins):,} Losses)")
    print(f"  Terminal Bal (Comp):  ${final_bal_comp:.2f}")
    print(f"  Terminal Bal (Fixed): ${final_bal_fixed:.2f}")
    print(f"  CAGR (Comp):          {cagr:.2f}%")
    ...
    for f in wfo_results:
        # Map fold index to approximate years
        start_yr = 2001 + (f['fold'] - 1) * 5
        end_yr = start_yr + 5
        print(f"  Fold {f['fold']} ({start_yr} - {end_yr}): Trades={f['trades_count']:,} | Win Rate={f['win_rate']:.2f}% | Sharpe={f['sharpe']:.4f} | Return={f['fold_return_pct']:.2f}%")
    print("="*80)
    
    # Save a comprehensive JSON report database
    master_deep_db = {
        'pair': 'GBPUSD_25Y',
        'metrics': {
            'active_days': n_days,
    ...
    out_file = DATA_DIR / "gbpusd_25y_deep_quant_results.json"
    with open(out_file, "w") as f:
        json.dump(master_deep_db, f, indent=2)
    print(f"\nSaved 25-year master quantitative report database to {out_file}")
