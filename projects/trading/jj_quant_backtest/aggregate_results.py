...
def aggregate(chunks: list[dict]) -> dict:
    ...
    # Deduplicate by (instrument, config_id) — keep best if reruns exist
    seen = {}
    for r in all_results:
        key = (r.get("instrument", ""), r.get("config_id", ""))
        if key not in seen or r.get("trade_count", 0) > seen[key].get("trade_count", 0):
            seen[key] = r
