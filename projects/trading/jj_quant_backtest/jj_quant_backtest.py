...
# ---------------------------------------------------------------------------
def _compute_sod_trough_dd(trades: list[dict], starting_balance: float) -> dict:
    """
    Compute start-of-day to trough drawdown for each trading day.
    
    For each day:
    - sod_balance = balance before the first trade of that day
    - trough = minimum balance during that day
    - dd_pct = (sod_balance - trough) / sod_balance * 100
    
    Returns dict with per-day stats and worst-case aggregates.
    """
    if not trades:
        return {"max_dd_pct": 0.0, "avg_dd_pct": 0.0, "days": 0}

    # Group trades by date
    from collections import OrderedDict
    days = OrderedDict()
    for t in trades:
        day = t.get("exit_time", "")[:10]  # YYYY-MM-DD
        if day not in days:
            days[day] = []
        days[day].append(t)
    ...
    for day, day_trades in days.items():
        # Start-of-day balance = balance_before first trade of the day
        sod_bal = day_trades[0].get("balance_after", 0) - day_trades[0].get("net", 0)
        if sod_bal <= 0:
            sod_bal = starting_balance

        # Trough = minimum balance during the day
        trough = min(t.get("balance_after", sod_bal) for t in day_trades)
        trough = max(trough, 1.0)  # floor at $1

        # Drawdown
        dd_dollar = sod_bal - trough
        dd_pct = (dd_dollar / sod_bal * 100.0) if sod_bal > 0 else 0.0

        dd_pcts.append(dd_pct)
        dd_dollars.append(dd_dollar)

    return {
        "max_dd_pct": round(max(dd_pcts), 2) if dd_pcts else 0.0,
        "max_dd_dollar": round(max(dd_dollars), 2) if dd_dollars else 0.0,
        "avg_dd_pct": round(float(np.mean(dd_pcts)), 2) if dd_pcts else 0.0,
        "avg_dd_dollar": round(float(np.mean(dd_dollars)), 2) if dd_dollars else 0.0,
        "days": len(days),
    }
...
def compute_quant_suite(
    ...
    # Start-of-day to trough drawdown
    try:
        results["sod_trough_dd"] = _compute_sod_trough_dd(
            trades, starting_balance
        )
    except Exception as e:
        logger.error("sod_trough_dd failed: %s", e)
        results["sod_trough_dd"] = {"error": str(e)}
