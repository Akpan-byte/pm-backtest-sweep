...
#
# 2026-08-01  subagent_persist-orderbook-snapshots
#   - Added prune_orb_state_and_book_buf() to drop _ORB_STATE and _BOOK_BUF
#     entries for expired/dropped markets that are no longer in discovery and
#     are not held as open positions.
# WHY: These module-level dicts grew without bound as new 5m markets rotated
#      every five minutes, contributing to the runner's 1 GB memory-restart
#      cycles.
#
# 2026-07-24  kilo
#   - Reordered _fetch_daily_klines to Hyperliquid 1d candles first, then Binance
#     1d, then Coinbase 1d.
#   - Reordered _fetch_8h_klines to Hyperliquid 8h candles first, then Binance 8h,
#     then Coinbase 1h aggregated to 8h.
# WHY: Luxembourg is geoblocked from Binance (403). Hyperliquid is the same direct
#      feed used for entry pricing and is reachable from Luxembourg, so it is the
#      natural primary source for indicator/gate data.
...
def _http_get_json(url, timeout=12, json_body=None, method="GET"):
    try:
        if method == "POST":
            body = __import__("json").dumps(json_body).encode("utf-8") if json_body else None
            req = urllib.request.Request(
                url,
                data=body,
                headers={"User-Agent": "orb-fade/1.0", "Content-Type": "application/json"},
                method="POST"
            )
        else:
        ...
        log.warning("HTTP %s failed for %s: %s", method, url, exc)
    return None
...
def _fetch_daily_klines(limit=70):
    ...
    Priority: Hyperliquid BTC 1d candles -> Binance BTCUSDT 1d -> Coinbase BTC-USD 1d.
    The current (forming) day bar is dropped so every returned bar is fully closed
    (no lookahead). Hyperliquid is first because it is the same direct feed used for
    entry pricing and is not geoblocked from Luxembourg.
    """
    today_open_ms = int(
        datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0).timestamp() * 1000
    )
    # --- Hyperliquid 1d candles ---
    try:
        now_ms = int(time.time() * 1000)
        start_ms = now_ms - (limit * 86400 * 1000)
        payload = {"type": "candleSnapshot", "req": {"coin": "BTC", "interval": "1d", "startTime": start_ms, "endTime": now_ms}}
        r = _http_get_json("https://api.hyperliquid.xyz/info", json_body=payload, method="POST")
        if r and isinstance(r, list):
            ...
            for k in r:
                ...
                if ot >= today_open_ms:
                    continue
                bars.append({"open": float(k.get("o", 0)), "high": float(k.get("h", 0)),
                             "low": float(k.get("l", 0)), "close": float(k.get("c", 0)), "open_ms": ot})
            bars.sort(key=lambda b: b["open_ms"])
            if len(bars) >= 25:
                return bars, "hyperliquid"
    except Exception as exc:  # noqa: BLE001
        log.warning("hyperliquid daily klines failed: %s", exc)
    # --- Binance fallback ---
    try:
...
def prune_orb_state_and_book_buf(current_cids, held_cids=None):
    """Remove memory retained for markets that have expired/dropped.

    The runner already computes the set of currently-discovered condition IDs
    each loop. Drop ORB state and full-depth book buffers for any market that
    is no longer current and is not held as an open position by any leg.
    """
    if not current_cids:
        return
    held = set(held_cids or [])
    keep = set(current_cids) | held
    # _ORB_STATE keys are (leg_id, market_id)
    stale_orb = [k for k in list(_ORB_STATE.keys()) if k[1] not in keep]
    for k in stale_orb:
        del _ORB_STATE[k]
    stale_book = [m for m in list(_BOOK_BUF.keys()) if m not in keep]
    for m in stale_book:
        del _BOOK_BUF[m]
    if stale_orb or stale_book:
        log.info("pruned stale state: orb=%d book=%d keep=%d",
                 len(stale_orb), len(stale_book), len(keep))
...
def _fetch_8h_klines(limit=90):
    """Ascending list of 8h bar dicts {open_ms,high,low,close,volume}.

    Priority: Hyperliquid BTC 8h candles -> Binance BTCUSDT 8h -> Coinbase 1h
    aggregated to 8h. The forming bar is kept; the publish-time lookup only ever
    selects CLOSED bars (close <= now). Empty list on failure -> the gate stays
    dormant (no flips), a safe degradation.
    """
    # --- Hyperliquid 8h candles ---
    try:
        now_ms = int(time.time() * 1000)
        start_ms = now_ms - (limit * 8 * 3600 * 1000)
        payload = {"type": "candleSnapshot", "req": {"coin": "BTC", "interval": "8h", "startTime": start_ms, "endTime": now_ms}}
        r = _http_get_json("https://api.hyperliquid.xyz/info", json_body=payload, method="POST")
        if r and isinstance(r, list):
            bars = []
            for k in r:
            ...
            bars.sort(key=lambda b: b["open_ms"])
            if len(bars) >= 10:
                log.info("gate 8h using hyperliquid: %d bars", len(bars))
                return bars
    except Exception as exc:  # noqa: BLE001
        log.warning("gate 8h hyperliquid failed: %s", exc)
    # --- Binance 8h fallback ---
    try:
        ...
        bars.sort(key=lambda b: b["open_ms"])
        if len(bars) >= 10:
            log.info("gate 8h using binance: %d bars", len(bars))
            return bars
    except Exception as exc:  # noqa: BLE001
        log.warning("gate 8h binance failed: %s", exc)
    # --- Coinbase 1h -> 8h aggregation fallback ---
    try:
        ...
        bars_1h.sort(key=lambda b: b["open_ms"])
        # Drop incomplete first slice so every 8h bar is fully formed.
        offset = len(bars_1h) % 8
        bars_1h = bars_1h[offset:]
        bars_8h = []
        i = 0
        while i < len(bars_1h):
            slice_8h = bars_1h[i:i + 8]
            if len(slice_8h) == 8:
                bars_8h.append({
                    "open_ms": slice_8h[0]["open_ms"],
                    "high": max(b["high"] for b in slice_8h),
                    "low": min(b["low"] for b in slice_8h),
                    "close": slice_8h[-1]["close"],
                    "volume": sum(b["volume"] for b in slice_8h),
                })
            i += 8
        if len(bars_8h) >= 10:
            log.info("gate 8h using coinbase 1h aggregate: %d bars", len(bars_8h))
            return bars_8h
        log.warning("gate 8h coinbase aggregate produced only %d bars", len(bars_8h))
        return bars_8h
    except Exception as exc:  # noqa: BLE001
        log.warning("gate 8h coinbase fallback failed: %s", exc)
    return []
...
def orb_fade_signal(
    ...
    **kwargs,
    ...
    if state.get("skip"):
        # Fix B: this market was first seen after its OR closed with no real range
        # ever built; it is permanently skipped for this leg (never seed a fake range).
    ...
    if state["or_high"] <= 0 or state["or_low"] == float("inf"):
        # Fix B: the process first saw this market AFTER its opening range closed and
        # NEVER built a real OR (only possible on a mid-contract restart). Do NOT seed
        # a fake range from spot (that fired a degenerate breakout on the next tick).
        # Permanently skip this market for this leg instead.
        state["skip"] = True
        # orlog: emit exactly one skip_late on the genuine first late-detection.
        # The early guard above keeps returning the plain no-event skip thereafter,
        # so subsequent ticks of the same market do NOT re-log.
