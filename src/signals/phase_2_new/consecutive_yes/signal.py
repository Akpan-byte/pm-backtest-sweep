...
#   - Created consecutive_yes signal module per INTERFACE.md and the active wave-2 strategy plan.
#   - 4 consecutive up ticks -> YES
...
def consecutive_yes_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: 4 consecutive up ticks -> YES"""
    ...
    last = spot_history[-5:]
    if all(last[i] > last[i-1] for i in range(1, 5)) and yp <= 0.80:
        entry = yp
        if 0.05 <= entry <= 0.85:
            ...
            return {
                ...
                "source": source,
                "reason": f"4 consecutive up ticks ({last[0]:.2f}->{last[-1]:.2f})",
            }
