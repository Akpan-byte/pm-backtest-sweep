...
#   - Created delayed_momentum_no signal module per INTERFACE.md and the active wave-2 strategy plan.
#   - last 3 ticks all down -> NO
...
def delayed_momentum_no_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: last 3 ticks all down -> NO"""
    ...
    last = spot_history[-4:]
    if all(last[i] < last[i-1] for i in range(1, 4)) and np_val <= 0.80:
        ...
        if 0.05 <= entry <= 0.85:
            ...
            return {
                ...
                "source": source,
                "reason": f"last 3 ticks down ({last[0]:.2f}->{last[-1]:.2f})",
            }
