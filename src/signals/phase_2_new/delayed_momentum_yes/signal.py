...
#   - Created delayed_momentum_yes signal module per INTERFACE.md and the active wave-2 strategy plan.
...
def delayed_momentum_yes_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: last 3 ticks all up -> YES"""
    ...
    if all(last[i] > last[i-1] for i in range(1, 4)) and yp <= 0.80:
        ...
        if 0.05 <= entry <= 0.85:
            ...
            return {
                ...
                "source": source,
                "reason": f"last 3 ticks up ({last[0]:.2f}->{last[-1]:.2f})",
            }
