...
#   - Created momentum_5t_yes signal module per INTERFACE.md and the active wave-2 strategy plan.
...
def momentum_5t_yes_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: spot higher than 5 ticks ago and yp <= 0.80 -> YES"""
    ...
    if spot_price > spot_history[-5] and yp <= 0.80:
        ...
        if 0.05 <= entry <= 0.85:
            ...
            return {
                ...
                "source": source,
                "reason": f"spot {spot_price:.2f} above 5-tick ago {spot_history[-5]:.2f}",
            }
