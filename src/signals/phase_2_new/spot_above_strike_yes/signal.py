...
#   - Created spot_above_strike_yes signal module per INTERFACE.md and the active wave-2 strategy plan.
...
def spot_above_strike_yes_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: spot above strike +20bps and yp <= 0.80 -> YES"""
    ...
    if spot_price > strike * 1.0002 and yp <= 0.80:
        ...
        if 0.05 <= entry <= 0.85:
            ...
            return {
                ...
                "source": source,
                "reason": f"spot {spot_price:.2f} > strike*1.0002 ({strike*1.0002:.2f}), yp {yp:.3f}",
            }
