...
#   - Created strike_distance_no signal module per INTERFACE.md and the active wave-2 strategy plan.
#   - spot < strike -10bps and np <= 0.75 -> NO
...
def strike_distance_no_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: spot < strike -10bps and np <= 0.75 -> NO"""
    ...
    if dist < -0.001 and np_val <= 0.75:
        entry = np_val
        if 0.05 <= entry <= 0.85:
            return {
                "triggered": True, "direction": "NO",
                "confidence": min(1.0, max(0.0, abs(dist) / 0.003)),
                "signal_price": spot_price, "entry_price": entry,
                "source": source,
                "reason": f"spot {spot_price:.2f} {dist:.5f} below strike {strike:.2f}",
            }
