...
def trend_family_regime_signal(**kwargs: Any) -> Dict[str, Any]:
    ...
    # Flip mode: when regime filter blocks a trade, take the opposite direction
    # instead of skipping. Only applies to directional gates (VWAP side).
    # Non-directional gates (vol, time-of-day) still skip as before.
    flip_on_filter = bool(cfg.get("flip_on_filter", False))
    ...
    # Non-directional: block ALL trades during bad hours. Flipping doesn't help.
    ...
    # ---- 2. volatility gate ----
    # Non-directional: block ALL trades in dead/choppy markets. Flipping doesn't help.
    ...
    if use_vwapside and not math.isnan(vwap_side) and not math.isnan(vwap_pct):
        ...
        if not aligned and vwap_pct >= vwap_pct_thr:
            if flip_on_filter:
                # FLIP: take the opposite direction instead of skipping.
                # If VWAP says "don't trade YES" → trade NO instead.
                # If VWAP says "don't trade NO" → trade YES instead.
                flipped = "NO" if direction == "YES" else "YES"
                entry_price = (
                    float(kwargs.get("no_ask", kwargs.get("np_val", 0.0)))
                    if flipped == "NO"
                ...
                if flip_entry_min <= entry_price <= flip_entry_max:
                    ...
                    sig["reason"] = (
                        f"FLIP: vwapside_8h against {direction} "
                        f"side={int(vwap_side)} pct={vwap_pct:.1f} -> {flipped}"
                    )
                    sig["source"] = "TREND_FAMILY_REGIME_FLIP"
                    sig["regime"] = {
                    ...
                    neutral["reason"] = (
                        f"FLIP blocked: {flipped} entry {entry_price} outside band"
                    )
                    return neutral
            else:
                neutral["reason"] = (
