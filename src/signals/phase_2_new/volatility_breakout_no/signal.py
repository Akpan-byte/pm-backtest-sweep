...
#   - Created volatility_breakout_no signal module per INTERFACE.md and the active wave-2 strategy plan.
#   - ATR10 > 1.2*ATR30 and spot at 10-tick low -> NO
...
def volatility_breakout_no_signal(**kwargs: Any) -> Dict[str, Any]:
    """Simple 5m BTC up/down signal: ATR10 > 1.2*ATR30 and spot at 10-tick low -> NO"""
    ...
    if atr10 > atr30 * 1.2 and spot_price <= min(spot_history[-10:]):
        entry = np_val
        if 0.05 <= entry <= 0.85:
            return {
                ...
                "source": source,
                "reason": f"vol expansion 10t ATR {atr10:.6f} vs 30t {atr30:.6f}, 10t low",
            }
