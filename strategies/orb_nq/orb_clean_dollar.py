#!/usr/bin/env python3
"""Clean ORB variant with optional dollar-based stop and target."""
...
def backtest_orb_clean_dollar(df: pd.DataFrame,
                               or_minutes: int,
                               stop_multiplier: float = 0.5,
                               dollar_stop: float = None,
                               dollar_target: float = None,
    ...
    for date, day_df in df.groupby("date"):
        ...
        for ts, bar in trade_df.iterrows():
            ...
            # Add dollar-based levels if provided
            if dollar_stop is not None:
                dollar_stop_price = entry_price - (dollar_stop / contract_value) * direction
                if direction == 1:
                    stop_price = max(stop_price, dollar_stop_price)
                else:
                    stop_price = min(stop_price, dollar_stop_price)

            target_price = None
            if dollar_target is not None:
            ...
            for ts2, bar2 in trade_df.loc[ts:].iloc[1:].iterrows():
                if direction == 1:
                    if bar2["low"] <= stop_price:
                        ...
                        exit_idx = ts2
                        break
                    if target_price is not None and bar2["high"] >= target_price:
                    ...
                    if bar2["high"] >= stop_price:
                        ...
                        exit_idx = ts2
                        break
                    if target_price is not None and bar2["low"] <= target_price:
                        exit_price = target_price
                        exit_reason = "target"
                        exit_idx = ts2
                        break
