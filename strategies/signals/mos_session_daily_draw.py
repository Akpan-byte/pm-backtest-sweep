# CHANGE_SUMMARY
# 2026-08-14  coder
#   - REWRITTEN as the FUTURES/normal-trading version of Blueprint 3.
#   - Direction is LONG/SHORT (no more YES/NO); entry is the market price
#     (spot_price at 00:00 UTC), not a binary ask.
#   - Removed Polymarket-only params (yp, np_val, yes_ask, no_ask, rem_sec,
#     max_entry_price, time_gate_seconds). Entry window stays 00:00 UTC MOS
#     session (via time_utils); doji / ESTABLISHED_MOVEMENT / PDH-PDL checks
#     and the 10-pip TP cap are unchanged.
#   - The Polymarket-flavored original is archived verbatim in
#     strategies/signals_polymarket/mos_session_daily_draw.py.
...
"""Blueprint 3 (FUTURES): 00:00 UTC MOS Session Daily Draw.
...
Signal kwargs:
  ...
  pip_value  : price units per pip for the asset (default 1.0 = 1 index point)
"""
...
def mos_session_daily_draw(
    ...
    **kwargs,
    ...
    direction_signal = "LONG" if direction == "UP" else "SHORT"
