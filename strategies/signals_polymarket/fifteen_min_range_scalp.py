...
Core logic:
  ...
  displacement candle in the HTF bias direction.
...
def fifteen_min_range_scalp(
    ...
    **kwargs,
    ...
    direction = "YES" if bias == "LONG" else "NO"
    ...
    # Entry at MC close per blueprint ("Entry: at MC close").  Fall back to
    # live bid/ask only if the MC close is unavailable or exceeds the cap.
    entry_price = mc["close"]
    if direction == "YES" and yes_ask is not None:
        entry_price = yes_ask  # live ask for YES side
    elif direction == "NO" and no_ask is not None:
        entry_price = no_ask   # live ask for NO side
