...
def negative_rr_consolidation_sweeper(
    ...
    **kwargs,
    ...
    candidates = [("NO", lvl) for lvl in eq["highs"]] + [("YES", lvl) for lvl in eq["lows"]]
    if not candidates:
    ...
    tp = eq_level
    sl = entry_price - SL_MULTIPLE_OF_TP * dist if direction == "YES" else entry_price + SL_MULTIPLE_OF_TP * dist
